...
publishes `mymacros-cli` to npm, pushes a release commit and tag, and creates a GitHub Release.

## Prerequisites

- Push access to `crcatala/mymacros-cli`
- An npm account with publish access to `mymacros-cli` (`npm whoami`)
...
The package is public and installs the `mymacros` command.
...
3. Run the complete local verification suite:
   ...
   This checks generated fixtures, builds TypeScript, runs unit tests and linting, and smoke-tests
   the exact `npm pack` tarball with production dependencies installed.
...
5. publishes `mymacros-cli` publicly to npm;
...
```bash
npm view mymacros-cli
npx -y mymacros-cli@latest --version
npx -y mymacros-cli@latest --help
```
